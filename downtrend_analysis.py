...
# 終値のDataFrameを展開して下降トレンドの日数をカウントする。
# 対象日の終値が前日より低い場合はdownTrendCountを加算する
def getDownTrendCount(targetDf, rows):
    i = 0
    downTrendCount = 0
    while i < rows - 1:
...
# 全銘柄指定の場合はtickerListを展開してgetClosePrice関数から各銘柄の終値を取得する。
# 個別銘柄の場合は対象銘柄の終値をgetClosePrice関数から取得する。
# 取得した終値はtimestampとともに辞書型closeDfに格納される。
# 辞書型closeDfをgetDownTrendCount関数に渡して連続下降日数を求める。
# 下降日数の結果は辞書型resultDictに格納
resultDict = {}
if ticker == "all_tickers":
    for t in tickerList:
        print("ticker: " + t)
        ...
        if rowsCount == 0:
            innerDict = {"downtrendDays": "", "msg": "data not found"}
        else:
            downtrendDays = getDownTrendCount(closeDf, rowsCount)
            innerDict = {"downtrendDays": downtrendDays, "msg": ""}
...
# 既存ファイルは上書きされる。ファイル名は"downtrend_日付"となる
d = datetime.now().date()
jsonFileName = "downtrend_" + str(d).replace("-", "") + ".json"
