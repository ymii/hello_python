...
def dailyInsert(aggDict):
    ...
    queryCount = int(aggDict["queryCount"])
    resultsCount = int(aggDict["resultsCount"])
...
# 抽出対象をすべての銘柄もしくは指定。
# すべての銘柄が選択された場合はgetUsTickersList関数から銘柄リストを求める。
usTickersList = None
print("[対象銘柄]:")
print("   すべて...0")
print("   銘柄指定...1")
...
if searchType == 0:
    ...
    usTickersListAll = getUsTickersList()
    while True:
        print("[銘柄コード]:")
        targetTicker = str(input())
        if targetTicker in usTickersListAll:
            usTickersList = [targetTicker]
            break
        else:
